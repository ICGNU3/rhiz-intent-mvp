// Agent Prioritizer Worker
// Runs Mapper agent for relationship prioritization
...
export interface AgentPrioritizeJobData {
  userId: string;
  goalId?: string;
  limit?: number;
  trigger?: 'scheduled' | 'manual' | 'goal_updated';
}

export async function processAgentPrioritize(job: any): Promise<any> {
  const { userId, goalId, limit = 15, trigger = 'scheduled' }: AgentPrioritizeJobData = job.data;

  console.log(`Processing agent prioritize job for user ${userId}, trigger: ${trigger}`);
  ...
  try {
    ...
    if (userSignals.length === 0) {
      console.log(`No signals found for user ${userId}, skipping prioritization`);
    ...
    console.log(`Agent prioritize completed for user ${userId}:`, action);

    return { success: true, action, trigger };

  } catch (error) {
    console.error(`Agent prioritize job failed for user ${userId}:`, error);
    throw error;
  }
}